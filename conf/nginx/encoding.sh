...
sudo start-stop-daemon --start --quiet --background --make-pidfile --pidfile /var/run/sync_bucket_live_${name}.pid --startas /opt/tizr/sync_bucket/sync_bucket_live.sh $var_unid ${name}
...
for ((i=1;i<=nb_selected_profile;i++)) do
	encoding_command_body+="-c:v libx264 -profile:v baseline -level 3.0 -g 10 -b:v ${encoding_profile[$i,3]} -vf scale=${encoding_profile[$i,1]}:-2 -c:a aac -ac 1 -strict -2 -b:a ${encoding_profile[$i,4]} -f flv \"rtmp://localhost app=hls playpath=$var_unid/${name}_level$i?unid=$var_unid\" "
done
